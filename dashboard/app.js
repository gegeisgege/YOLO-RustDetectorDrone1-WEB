...
const state = {
  ...
  config: {
    wsUrl: localStorage.getItem('pw_ws') || '',
...
const map = L.map('map', { zoomControl: true, attributionControl: false }).setView(
  [state.config.lat, state.config.lon], 15
);
...
function connectWS(url) {
  ...
  state.ws.onmessage = (ev) => {
    // Binary frame → video overlay data (JSON-encoded detections from RPi)
...
function renderOverlay(detections) {
  const w = feedImg.clientWidth || feedImg.width;
  const h = feedImg.clientHeight || feedImg.height;
  canvas.width = w;
  canvas.height = h;
  ctx.clearRect(0, 0, w, h);

  const nw = feedImg.naturalWidth || 640;
  const nh = feedImg.naturalHeight || 480;
  const sx = w / nw;
  const sy = h / nh;
...
function renderStats() {
  ...
  document.getElementById('stat-conf').textContent =
    total ? `${(confSum / total * 100).toFixed(0)}%` : '--%';
...
function renderLog() {
  ...
  const rows = state.detections.slice(0, 200).map(d => {
    ...
    const confClass = d.confidence >= 0.8 ? 'conf-high' : d.confidence >= 0.6 ? 'conf-mid' : 'conf-low';
    return `<tr>
      <td>${d.time}</td>
...
document.getElementById('btn-settings').addEventListener('click', () => drawer.classList.toggle('open'));
document.getElementById('btn-close-settings').addEventListener('click', () => drawer.classList.remove('open'));
...
document.getElementById('btn-demo').addEventListener('click', () => {
  ...
  classes.forEach((cls, i) => {
    setTimeout(() => {
      ...
      handleDetections(
        [{ class: cls, confidence: confs[i], bbox }],
        lat, lon,
...
document.getElementById('btn-export').addEventListener('click', () => {
  ...
  const rows = state.detections.map(d =>
    `${d.time},${d.cls},${d.confidence.toFixed(4)},${d.lat ?? ''},${d.lon ?? ''}`
  ).join('\n');
...
// ─── Poll /detections endpoint (fallback if no WS) ───────────────────────────
  ...
  pollInterval = setInterval(async () => {
    try {
      ...
      if (Array.isArray(data) && data.length) {
        data.forEach(item => handleGPS(item));
        setConnected(true);
      }
    } catch (_) {}
  }, 2000);
...
// If WS URL looks like http, use polling instead
document.getElementById('btn-connect').addEventListener('click', () => {
  const ws = document.getElementById('cfg-ws').value.trim();
  if (ws.startsWith('http')) startPolling(ws);
}, true);

// ─── Auto-connect on load ─────────────────────────────────────────────────────
if (state.config.wsUrl) connectWS(state.config.wsUrl);
